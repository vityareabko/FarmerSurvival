...
namespace Fraktalia.VoxelGen
{
    ...
    public static class VoxelMath
    {
		...
		public static Bounds CalculateRotatedBounds(VoxelGenerator generator, Vector3 center, Vector3 size, Quaternion rotation)
		{
			Vector3 extends_max = size * 0.5f;
			Vector3 extends_min = -extends_max;
			...
			Vector3 rotatedsize;
			rotatedsize.x = Mathf.Max(extends_max.x, extends_corner.x, extends_corner3.x, extends_corner5.x);
			rotatedsize.y = Mathf.Max(extends_max.y, extends_corner.y, extends_corner3.y, extends_corner5.y);
			rotatedsize.z = Mathf.Max(extends_max.z, extends_corner.z, extends_corner3.z, extends_corner5.z);

			Vector3 rotatedcenter = matrix.MultiplyPoint(center);

			return new Bounds(rotatedcenter, rotatedsize);
		}
		...
		public static Vector3 ChunkHashToLocalPosition(Vector3Int chunkHash, float volumeSize)
        {
			Vector3 localPosition = new Vector3(chunkHash.x * volumeSize, chunkHash.y * volumeSize, chunkHash.z * volumeSize);
			return localPosition;
		}

		public static Vector3Int LocalPositionToChunk(Vector3 localPosition, float volumeSize)
		{	
