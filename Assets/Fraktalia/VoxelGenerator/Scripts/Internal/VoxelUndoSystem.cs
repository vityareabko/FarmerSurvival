...
namespace Fraktalia.VoxelGen
{
	...
	{
		public const int MAXIMUMUNDOVOXELS = 10000000; 
		...
		{
			private FNativeList<NativeVoxelModificationData> PreviousData = new FNativeList<NativeVoxelModificationData>();
			private FNativeList<NativeVoxelModificationData_Inner> PreviousData_Inner = new FNativeList<NativeVoxelModificationData_Inner>();

			private FNativeList<NativeVoxelModificationData> ModificationData = new FNativeList<NativeVoxelModificationData>();
			private FNativeList<NativeVoxelModificationData_Inner> ModificationData_Inner = new FNativeList<NativeVoxelModificationData_Inner>();
			...
			public void AddInner(FNativeList<NativeVoxelModificationData_Inner> data)
            {			
				if (ModificationData_Inner.Length > MAXIMUMUNDOVOXELS)
				{
					if(!CapReached)
                    {
						CapReached = true;
						Debug.LogWarning("Undo Manifest Cap Reached. Cap is at " + MAXIMUMUNDOVOXELS + " voxels");
                    }
					return;
				}
				ModificationData_Inner.AddRange(data);

			}

			public void AddPreviousInner(FNativeList<NativeVoxelModificationData_Inner> data)
			{
				if (PreviousData_Inner.Length > MAXIMUMUNDOVOXELS)
				{
					if (!CapReached)
					{
						CapReached = true;
						Debug.LogWarning("Undo Manifest Cap Reached. Cap is at" + MAXIMUMUNDOVOXELS + " voxel");
					}
					return;
				}
				PreviousData_Inner.AddRange(data);
			}
		}
	}
