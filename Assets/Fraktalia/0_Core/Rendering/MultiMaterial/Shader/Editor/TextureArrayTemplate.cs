...
namespace Fraktalia.Core.FraktaliaAttributes
{
	...
	public class TextureArrayTemplate : ScriptableObject
	{
		...
		public Texture2DArray CreateTextureArray(Texture2D[] ordinaryTextures, Texture2D nullTexture, string FileName, Color[] _Colors = null, SmoothnessSource[] _SmoothMode = null)
		{
			...
			Texture2DArray texture2DArray = new Texture2DArray(width, height, ordinaryTextures.Length, TextureFormat.DXT5, true, false);
			...
			for (int i = 0; i < ordinaryTextures.Length; i++)
			{
				...
				// Create a new Texture2D in DXT5 format
				Texture2D tempTexture = new Texture2D(texture2D.width, texture2D.height, TextureFormat.RGBA32, true);
				tempTexture.SetPixels(pixels);	
				tempTexture.Apply();
				tempTexture.Compress(false);
			
				Graphics.CopyTexture(tempTexture, 0, texture2DArray, i);
