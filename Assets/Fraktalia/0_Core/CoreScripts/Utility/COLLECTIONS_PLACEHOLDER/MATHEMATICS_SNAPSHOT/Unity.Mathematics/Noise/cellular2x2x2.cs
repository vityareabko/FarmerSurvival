...
namespace Fraktalia.Core.Mathematics
{
    public static partial class Fnoise
    {
        ...
        public static Ffloat2 cellular2x2x2(Ffloat3 P)
        {
            ...
            Ffloat3 Pf = frac(P);
            Ffloat4 Pfx = Pf.x + float4(0.0f, -1.0f, 0.0f, -1.0f);
            Ffloat4 Pfy = Pf.y + float4(0.0f, 0.0f, -1.0f, -1.0f);
            Ffloat4 p = permute(Pi.x + float4(0.0f, 1.0f, 0.0f, 1.0f));
            p = permute(p + Pi.y + float4(0.0f, 0.0f, 1.0f, 1.0f));
            Ffloat4 p1 = permute(p + Pi.z); // z+0
            Ffloat4 p2 = permute(p + Pi.z + float4(1.0f,1.0f,1.0f,1.0f)); // z+1
            Ffloat4 ox1 = frac(p1 * K) - Ko;
            Ffloat4 oy1 = mod7(floor(p1 * K)) * K - Ko;
            Ffloat4 oz1 = floor(p1 * K2) * Kz - Kzo; // p1 < 289 guaranteed
            Ffloat4 ox2 = frac(p2 * K) - Ko;
            Ffloat4 oy2 = mod7(floor(p2 * K)) * K - Ko;
            Ffloat4 oz2 = floor(p2 * K2) * Kz - Kzo;
            Ffloat4 dx1 = Pfx + jitter * ox1;
            Ffloat4 dy1 = Pfy + jitter * oy1;
            Ffloat4 dz1 = Pf.z + jitter * oz1;
            Ffloat4 dx2 = Pfx + jitter * ox2;
            Ffloat4 dy2 = Pfy + jitter * oy2;
            Ffloat4 dz2 = Pf.z - 1.0f + jitter * oz2;
            Ffloat4 d1 = dx1 * dx1 + dy1 * dy1 + dz1 * dz1; // z+0
            Ffloat4 d2 = dx2 * dx2 + dy2 * dy2 + dz2 * dz2; // z+1

            // Sort out the two smallest distances (F1, F2)

            // Do it right and sort out both F1 and F2
            Ffloat4 d = min(d1,d2); // F1 is now in d
