...
namespace Fraktalia.Core.Mathematics
{
    public static partial class Fnoise
    {
        ...
        public static Ffloat2 cellular(Ffloat2 P)
        {
            ...
            Ffloat2 Pf = frac(P);
            Ffloat3 oi = float3(-1.0f, 0.0f, 1.0f);
            Ffloat3 of = float3(-0.5f, 0.5f, 1.5f);
            Ffloat3 px = permute(Pi.x + oi);
            Ffloat3 p = permute(px.x + Pi.y + oi); // p11, p12, p13
            Ffloat3 ox = frac(p * K) - Ko;
            Ffloat3 oy = mod7(floor(p * K)) * K - Ko;
            Ffloat3 dx = Pf.x + 0.5f + jitter * ox;
            Ffloat3 dy = Pf.y - of + jitter * oy;
            Ffloat3 d1 = dx * dx + dy * dy; // d11, d12 and d13, squared
            p = permute(px.y + Pi.y + oi); // p21, p22, p23
            ox = frac(p * K) - Ko;
            oy = mod7(floor(p * K)) * K - Ko;
            dx = Pf.x - 0.5f + jitter * ox;
            dy = Pf.y - of + jitter * oy;
            Ffloat3 d2 = dx * dx + dy * dy; // d21, d22 and d23, squared
            p = permute(px.z + Pi.y + oi); // p31, p32, p33
            ox = frac(p * K) - Ko;
            oy = mod7(floor(p * K)) * K - Ko;
            dx = Pf.x - 1.5f + jitter * ox;
            dy = Pf.y - of + jitter * oy;
            Ffloat3 d3 = dx * dx + dy * dy; // d31, d32 and d33, squared
            // Sort out the two smallest distances (F1, F2)
            Ffloat3 d1a = min(d1, d2);
