...
namespace Fraktalia.Core.Math
{
    ...
    public static class MathUtilities
    {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static int DivideByPowerOf2(int x, int n)
		{
			int number = n;

			// Calculate the power of two (exponent) using bit shifting
			int powerOfTwo = 0;
			while ((number & 1) == 0)
			{
				number >>= 1;
		...
		public static int Convert3DSizeToShift(int sizePowerofTwo)
		{	
			return RightmostBitPosition(sizePowerofTwo); // Return the highest valid shift value
		}
	}
}
