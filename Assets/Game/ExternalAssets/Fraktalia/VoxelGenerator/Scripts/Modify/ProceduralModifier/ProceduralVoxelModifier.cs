...
namespace Fraktalia.VoxelGen.Modify.Procedural
{
	...
	public class ProceduralVoxelModifier : MonoBehaviour
	{
		...
		//Must be Local Space of TargetGenerator
		...
		public void CalculateDisplacement(Vector3 worldPosition, VoxelGenerator target)
		{
			float voxelsize = target.GetVoxelSize(Depth);
		...
		public void ApplyProceduralModifier(bool inversed = false)
		{
			...
			for (int i = 0; i < modifiedgenerators.Count; i++)
			{
				...
				boundary = GetRegionBounds(boundary);
		...
		public virtual Bounds GetRegionBounds(Bounds calculatedBounds)
        {
	...
	public class ProceduralVoxelModifierEditor : Editor
	{
		private void OnSceneGUI()
		{
			...
			if (!myTarget.TargetGenerator) return;
			if (!myTarget.IsSave()) return;
