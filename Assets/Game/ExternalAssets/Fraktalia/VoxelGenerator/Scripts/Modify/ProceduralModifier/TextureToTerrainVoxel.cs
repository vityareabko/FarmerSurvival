	...
	public class TextureToTerrainVoxel : ProceduralVoxelModifier
	{
		...
		public override void OnDrawGizmosSelected()
		{
			if (TargetGenerator)
			{
				...
				Gizmos.DrawCube(bound.center - offset, bound.size);
		...
		public override void EvaluateVoxelInfo(Vector3 start, Vector3 end)
		{
			...
			for (int x = 0; x < boundaryvoxelsize.x; x++)
			{
				for (int y = 0; y < boundaryvoxelsize.y; y++)
				{
					...
					{
						Vector3 localPosition = start + new Vector3(voxelsize * x, voxelsize * y, voxelsize * z) - offset;
		...
		public override Bounds CalculateBounds()
		{
			...
			bound.min = Vector3.zero - new Vector3(0, BottomExtension, 0) + offset;
			Vector3 size = new Vector3(Size.x, 0, Size.y);
			size.y += TopExtension;
			bound.max = size + offset;			
			bound.size = bound.size * 1.1f;

			return bound;
		}

        public override Bounds GetRegionBounds(Bounds calculatedBounds)
        {
			Vector3 offset = TargetGenerator.transform.worldToLocalMatrix.MultiplyPoint3x4(transform.position);
			calculatedBounds.center -= offset;
			Vector3 center = calculatedBounds.center;
			Vector3 max = calculatedBounds.max;
			Vector3 min = calculatedBounds.min;

			center = transform.localToWorldMatrix.MultiplyPoint3x4(center);
			center = targetgenerator_worldtolocalmatrix.MultiplyPoint3x4(center);
			min = transform.localToWorldMatrix.MultiplyPoint3x4(min);
			min = targetgenerator_worldtolocalmatrix.MultiplyPoint3x4(min);
			max = transform.localToWorldMatrix.MultiplyPoint3x4(max);
			max = targetgenerator_worldtolocalmatrix.MultiplyPoint3x4(max);

			calculatedBounds.center = center;
