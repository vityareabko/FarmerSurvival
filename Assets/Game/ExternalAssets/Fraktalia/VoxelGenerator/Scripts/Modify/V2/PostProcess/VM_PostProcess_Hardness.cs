...
namespace Fraktalia.VoxelGen.Modify
{
	...
	public class VM_PostProcess_Hardness : VM_PostProcess
	{
		...
		[Button("Apply Histogramm from Curve")]
		public bool _applyHistogrammFromCurve;
		...
		[Button("Apply MinID/MaxID hardness")]
		public bool _applyHistogrammFromMinMax;
		...
		{
			base.DrawEditorPreview(modifier, isSafe, worldPosition, worldNormal);

			if (_applyHistogrammFromCurve)
			{
				CreateHistogramm(HistogrammCurve);
				_applyHistogrammFromCurve = false;
			}

			if (_applyHistogrammFromMinMax)
			{
				SetHardness(MinID, MaxID, MinMaxHardness);
				_applyHistogrammFromMinMax = false;
			}
		}
	}
