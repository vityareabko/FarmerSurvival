...
namespace Fraktalia.VoxelGen.Visualisation
{
	...
	public unsafe class SurfaceModifier_CubeUV : SurfaceModifier
	{
		...
		protected override void initializeModule()
		{
			...
			{
				jobs[i].tan1 = ContainerStaticLibrary.GetVector3FNativeList("Tangent1", i);
				jobs[i].tan2 = ContainerStaticLibrary.GetVector3FNativeList("Tangent2", i);
			}
		}
		...
		public override IEnumerator beginCalculationasync(float cellSize, float voxelSize)
		{
			for (int coreindex = 0; coreindex < HullGenerator.activeCores; coreindex++)
			{
				...
				jobs[coreindex].Mode = Mode;
				jobs[coreindex].Offset = (OffsetDirection.normalized * OffsetMultiplier);
				jobs[coreindex].OffsetMultiplier = OffsetMultiplier;
		...
		public struct CubeUVJob : IJob
		{
            ...
            public void Execute()
			{
                ...
                switch (Mode)
                {
                    case UVMode.CubeUV:
						...
						break;
                    case UVMode.CuveUV_Alternative:
						CalculateCubemap_Alternate();
						break;
                    case UVMode.SphereUV:
			...
			public void CalculateCubemap_Alternate()
			{
				// Ensure UV array has enough capacity
				if (uvArray.Length != verticeArray.Length)
				{
					uvArray.Resize(verticeArray.Length, NativeArrayOptions.UninitializedMemory);
				}

				if (verticeArray.Length == 0) return;

				Vector3 minBounds = verticeArray[0];
				Vector3 maxBounds = verticeArray[0];
				...
				for (int i = 1; i < verticeArray.Length; i++)
				{
					...
					minBounds.y = Mathf.Min(minBounds.y, vertex.y);
					minBounds.z = Mathf.Min(minBounds.z, vertex.z);

					// Update max bounds
					maxBounds.x = Mathf.Max(maxBounds.x, vertex.x);
					maxBounds.y = Mathf.Max(maxBounds.y, vertex.y);
					maxBounds.z = Mathf.Max(maxBounds.z, vertex.z);
				}
				Bounds boundingBox = new Bounds((maxBounds + minBounds) * 0.5f, maxBounds - minBounds);

				for (int i = 0; i < verticeArray.Length; i++)
				{
					Vector3 vertex = verticeArray[i] + Offset;
					Vector3 normal = normalArray[i];

					// Determine which face of the cube the vertex belongs to
					Vector2 uv;

					// Normalize the vertex position to ensure consistent scaling
					Vector3 normalizedPosition = vertex.normalized;

					// Calculate UV coordinates based on the direction of the normalized vertex position
					if (Mathf.Abs(normal.x) > Mathf.Abs(normal.y) && Mathf.Abs(normal.x) > Mathf.Abs(normal.z))
					{
						// X face
						uv = new Vector2(normalizedPosition.z, normalizedPosition.y) / Mathf.Abs(normalizedPosition.x);
					}
					else if (Mathf.Abs(normal.y) > Mathf.Abs(normal.x) && Mathf.Abs(normal.y) > Mathf.Abs(normal.z))
					{
						// Y face
						uv = new Vector2(normalizedPosition.x, normalizedPosition.z) / Mathf.Abs(normalizedPosition.y);
					}
					else
					{
						// Z face
						uv = new Vector2(normalizedPosition.x, normalizedPosition.y) / Mathf.Abs(normalizedPosition.z);
					}

					// Set UV coordinate
					uvArray[i] = uv * UVPower * OffsetMultiplier;
				}
			}
			...
			public void CalculateSpheremap(int count)
			{
				for (int i = 0; i < count; i++)
				{
					Vector3 n = (verticeArray[i] + Offset).normalized;

					float u = (Mathf.Atan2(n.z, n.x) / (2f * Mathf.PI));
					float v = (Mathf.Asin(n.y) / Mathf.PI) + 0.5f;

					uvArray.Add(new Vector2(u, v) * UVPower * OffsetMultiplier*10);
				}
			}
		...
		{
			float sum = UVPower + (int)Mode + OffsetMultiplier + OffsetDirection.sqrMagnitude;
