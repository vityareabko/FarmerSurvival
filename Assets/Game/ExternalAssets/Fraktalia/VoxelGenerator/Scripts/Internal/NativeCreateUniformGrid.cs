...
namespace Fraktalia.VoxelGen.Visualisation
{
	...
	[BurstCompile]
	public unsafe struct NativeCreateUniformGrid_V3 : IJobParallelFor
	{
		[NativeDisableParallelForRestriction]
		public NativeVoxelTree data;
		...
		public void Execute(int index)
		{
			...
			int fy = positionoffset.y + ((position.y - 1) << voxelSizeBitPosition);
			int fz = positionoffset.z + ((position.z - 1) << voxelSizeBitPosition);
			float rawvalue = data._PeekVoxelId_InnerCoordinate(fx, fy, fz, 10, Shrink, 0, 0, ShrinkMax);
	...
	public unsafe struct GPUToMesh_v2 : IJob
	{
		...
		public void Execute()
		{
			...
			if (MergeUV == 1)
			{
				...
				if (texturedata_UV3.IsCreated && texturedata_UV5.IsCreated)
				{
					for (int i = 0; i < count; i++)
					{
						Vector3 vertex = verticeArray[i];
						float texturevalue = texturedata_UV3._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, 0) * TexturePowerUV3;
						float texturevalue2 = texturedata_UV5._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, 0) * TexturePowerUV5;
						uv3Array.Add(new Vector2(texturevalue, texturevalue2));
					}
				}
				else if (texturedata_UV3.IsCreated)
				{
					for (int i = 0; i < count; i++)
					{
						Vector3 vertex = verticeArray[i];
				...
				if (texturedata_UV4.IsCreated && texturedata_UV6.IsCreated)
				{
					for (int i = 0; i < count; i++)
					{
						Vector3 vertex = verticeArray[i];
						float texturevalue = texturedata_UV4._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, 0) * TexturePowerUV4;
						float texturevalue2 = texturedata_UV6._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, 0) * TexturePowerUV6;
						uv4Array.Add(new Vector2(texturevalue, texturevalue2));
					}
				}
				else if (texturedata_UV4.IsCreated)
				{
					for (int i = 0; i < count; i++)
					{
						Vector3 vertex = verticeArray[i];
