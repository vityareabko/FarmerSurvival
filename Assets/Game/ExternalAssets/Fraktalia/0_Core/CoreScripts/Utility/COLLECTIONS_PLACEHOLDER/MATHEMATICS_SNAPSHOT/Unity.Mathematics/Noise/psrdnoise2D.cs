...
namespace Fraktalia.Core.Mathematics
{
    public static partial class Fnoise
    {
        ...
        public static Ffloat3 psrdnoise(Ffloat2 pos, Ffloat2 per, float rot)
        {
            // Hack: offset y slightly to hide some rare artifacts
            pos.y += 0.01f;
            // Skew to hexagonal grid
            Ffloat2 uv = float2(pos.x + pos.y * 0.5f, pos.y);

            Ffloat2 i0 = floor(uv);
            Ffloat2 f0 = frac(uv);
            // Traversal order
            Ffloat2 i1 = (f0.x > f0.y) ? float2(1.0f, 0.0f) : float2(0.0f, 1.0f);

            // Unskewed grid points in (x,y) space
            Ffloat2 p0 = float2(i0.x - i0.y * 0.5f, i0.y);
            Ffloat2 p1 = float2(p0.x + i1.x - i1.y * 0.5f, p0.y + i1.y);
            Ffloat2 p2 = float2(p0.x + 0.5f, p0.y + 1.0f);
            ...
            Ffloat3 xw = fmod(float3(p0.x, p1.x, p2.x), per.x);
            Ffloat3 yw = fmod(float3(p0.y, p1.y, p2.y), per.y);
            Ffloat3 iuw = xw + 0.5f * yw;
            Ffloat3 ivw = yw;

            // Create gradients from indices
            Ffloat2 g0 = rgrad2(float2(iuw.x, ivw.x), rot);
            Ffloat2 g1 = rgrad2(float2(iuw.y, ivw.y), rot);
            Ffloat2 g2 = rgrad2(float2(iuw.z, ivw.z), rot);

            // Gradients math.dot vectors to corresponding corners
            // (The derivatives of this are simply the gradients)
            Ffloat3 w = float3(dot(g0, d0), dot(g1, d1), dot(g2, d2));

            // Radial weights from corners
            // 0.8 is the square of 2/math.sqrt(5), the distance from
            // a grid point to the nearest simplex boundary
            Ffloat3 t = 0.8f - float3(dot(d0, d0), dot(d1, d1), dot(d2, d2));

            // Partial derivatives for analytical gradient computation
            Ffloat3 dtdx = -2.0f * float3(d0.x, d1.x, d2.x);
            Ffloat3 dtdy = -2.0f * float3(d0.y, d1.y, d2.y);
            ...
            Ffloat2 dt0 = float2(dtdx.x, dtdy.x) * 4.0f * t3.x;
            Ffloat2 dn0 = t4.x * g0 + dt0 * w.x;
            Ffloat2 dt1 = float2(dtdx.y, dtdy.y) * 4.0f * t3.y;
            Ffloat2 dn1 = t4.y * g1 + dt1 * w.y;
            Ffloat2 dt2 = float2(dtdx.z, dtdy.z) * 4.0f * t3.z;
            Ffloat2 dn2 = t4.z * g2 + dt2 * w.z;
        ...
        /// <param name="per">The x and y period, where per.x is a positive integer and per.y is a positive even integer.</param>
        /// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
        public static Ffloat3 psrdnoise(Ffloat2 pos, Ffloat2 per)
        {
        ...
        public static float psrnoise(Ffloat2 pos, Ffloat2 per, float rot)
        {
        ...
        /// <param name="per">The x and y period, where per.x is a positive integer and per.y is a positive even integer.</param>
        /// <returns>Noise value.</returns>
        public static float psrnoise(Ffloat2 pos, Ffloat2 per)
        {
        ...
        /// <param name="rot">Angle to rotate the gradients.</param>
        /// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
        public static Ffloat3 srdnoise(Ffloat2 pos, float rot)
        {
            // Offset y slightly to hide some rare artifacts
            pos.y += 0.001f;
            // Skew to hexagonal grid
            ...
            Ffloat3 x = float3(p0.x, p1.x, p2.x);
            Ffloat3 y = float3(p0.y, p1.y, p2.y);
            Ffloat3 iuw = x + 0.5f * y;
            Ffloat3 ivw = y;
        ...
        /// <param name="pos">Input (x,y) coordinate.</param>
        /// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
        public static Ffloat3 srdnoise(Ffloat2 pos)
        {
        ...
        /// <param name="rot">Angle to rotate the gradients.</param>
        /// <returns>Noise value.</returns>
        public static float srnoise(Ffloat2 pos, float rot)
        {
            // Offset y slightly to hide some rare artifacts
            pos.y += 0.001f;
            // Skew to hexagonal grid
        ...
        /// <param name="pos">Input (x,y) coordinate.</param>
        /// <returns>Noise value.</returns>
        public static float srnoise(Ffloat2 pos)
        {
